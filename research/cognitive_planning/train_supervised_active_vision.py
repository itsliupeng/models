...
def unroll_policy_for_eval(
    ...
    with tf.gfile.Open(output_path, 'w') as f:
        print
        'saving path information to {}'.format(output_path)
...
def test():
    ...
    with sv.managed_session(
        ...
        while not sv.should_stop():
            ...
            '--------------------'
            print
            'evaluating checkpoint {}'.format(new_checkpoint)
            ...
            for test_iter in range(FLAGS.test_iters):
                print
                'evaluating {} of {}'.format(test_iter, FLAGS.test_iters)
