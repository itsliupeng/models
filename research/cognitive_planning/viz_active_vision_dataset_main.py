...
def human(env, targets):
    ...
    for _ in range(20):
        ...
        while True:
            print
            'distance = ', obs[task_env.ModalityTypes.DISTANCE]
            steps += 1
            ...
            if action == 'stop':
                print
                'dist to goal: {}'.format(len(env.path_to_goal()) - 2)
                break
            obs, reward, done, info = env.step(action)
            print
            'reward = {}, done = {}, success = {}'.format(
...
def visualize(env, input_folder, output_root_folder):
    ...
    for i, npy_file in enumerate(npy_files):
        print
        'saving images {}/{}'.format(i, len(npy_files))
...
def evaluate_folder(env, folder_path):
    ...
    def evaluate_iteration(folder):
        ...
        for c in eval_stats:
            if not eval_stats[c]:
                print
                'incomplete eval {}: empty class {}'.format(folder_path, c)
                return None
    ...
    print
    '{} folders found'.format(len(checkpoint_folders))
    print
    '------------------------'
    all_iters = []
    all_accs = []
    for i, folder in enumerate(checkpoint_folders):
        print
        'processing {}/{}'.format(i, len(checkpoint_folders))
        ...
        if eval_stats is None:
            ...
            print
            'result ', iter_no, eval_stats['mean']
    ...
    print
    'best result at iteration {} was {}'.format(all_iters[idx],
                                                all_accs[idx])
    ...
    print
    'generating gifs and images for {}'.format(best_iteration_folder)
